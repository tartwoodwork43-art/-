...
function mediaHtml(path, alt = '', className = '') {
  if (!path) return '';
  if (isVideo(path)) {
    return `<video class="${className}" controls playsinline preload="metadata"><source src="${path}" type="video/mp4">הדפדפן שלך לא תומך בהצגת וידאו.</video>`;
  }
  return `<img class="${className}" src="${path}" alt="${alt}">`;
}

async function init() {
  const d = await fetch('content/site.json').then(r => r.json());
  ...
  if (d.settings.logo) $('#logoImg').src = d.settings.logo;
  ...
  d.categories.forEach(c => {
    ...
    a.href = `#cat-${c.id}`;
    a.innerHTML = `${mediaHtml(c.cover, c.name)}<div class="overlay"><strong>${c.name}</strong><span>${c.intro}</span></div>`;
  ...
  (d.testimonials || []).forEach(t => {
    ...
    card.innerHTML = `<h3>${t.name}</h3><p>${'★'.repeat(Number(t.rating) || 5)}</p><p>${t.text}</p>`;
